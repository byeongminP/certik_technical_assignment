...
// EncodeCertIDs converts cert ids into a binary representation.
func EncodeCertIDs(o []uint64) []byte {
	bz := make([]byte, 8*len(o))
	for i, val := range o {
		binary.LittleEndian.PutUint64(bz[8*i:], val)
	}
	return bz
}
